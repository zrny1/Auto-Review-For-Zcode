...
- 修改性质：文档更新（wiki 时效性补齐）+ 安全排查
- 背景/需求：0.2.4/0.2.5 两轮变更后 wiki 部分文档未同步；用户要求全量检查文档时效性、排查文档与提交历史中的敏感信息、并给出 GitHub Wiki 迁移方案。
- 方案/决策：纯文档周期不改代码不升版本；原生框分析显示的旧表述修正为逆向实证的精确版（自动编辑模式升级路径显示 reason、default 模式同向叠加丢弃）；把本轮踩到的两个环境坑（缓存仅启动同步、PS5.1 无 BOM UTF-8）与 PermissionRequest 选型结论沉淀进二次开发文档；敏感信息扫描用密钥形态特征（sk-/tp-/32hex.后缀）+ 个人路径 + 用户名 + 邮箱/手机号双通道（工作区 + git log --all -p 全 44 提交）。
- 影响范围：wiki 七个文档；无代码变更。
- 验证结果：文档与当前代码行为逐处对照一致；工作区与全部提交历史敏感信息零命中（脚本仅 example.com 占位与运行时解析 baseURL，无硬编码密钥）。
