...
flowchart TD
    ...
    SA --> G{④ 缓存层<br/>sha256 稳定键 + 附件摘要加盐}
    G -- 命中未过期 --> R[复用上次决策]
    G -- 未命中 --> H[⑤ 安全子 agent<br/>上下文干净的 LLM 调用<br/>载荷含可选脚本附件块]
    H -- "allow" --> Y1[放行 + 一句话分析]
...
生成同一键，等价调用命中同一缓存。附件摘要 = 各文件路径与内容 sha256 拼接（0.2.3）：
脚本内容变更后旧结论自动失效；无附件时与旧版键完全一致（历史缓存/白名单条目兼容）。
