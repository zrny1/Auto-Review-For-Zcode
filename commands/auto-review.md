...
- **无参数 或 `status`**：运行 `node "$CTL" status`，把输出整理成简洁的中文状态汇报（开关、审查哪些工具、provider/model、规则条数、提示词是否自定义、脚本送审开关），并附一行常用用法提示。
- **`gui`**：运行 `node "$CTL" gui`——打开**图形配置界面**（深色主题设置窗口）：总开关、审查对话框开关、审查工具勾选、脚本内容送审开关、provider/模型下拉、超时缓存、危险规则的添加/删除/测试/恢复出厂、安全策略提示词编辑器（打开后显示全文可编辑、保存立即生效），窗口内点击保存即生效。提醒用户这是命令式配置的图形替代。
...
- **`set <key> <value>`**：运行 `node "$CTL" set <key> <value>`（value 用引号包裹原样传递）。校验失败时把错误原样转述并给出合法取值说明：
  ...
  - `inspect_scripts`: true/false（脚本内容随命令送审——开启后 python/node/bash 等调用的脚本文件内容随载荷一并审查，相对路径按工作目录解析；关闭时安全子 agent 只看命令文本）
  - `script_max_bytes`: 1000~100000（脚本送审单文件读取上限，最多附加 3 个文件）
